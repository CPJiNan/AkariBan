...
import com.github.cpjinan.plugin.playerbanex.internal.manager.ConfigManager.options
...
object LoginEvent {
    ...
    fun onPlayerLogin(event : AsyncPlayerPreLoginEvent) {
        if (banlist.getBoolean("${event.name}.enable") && LocalDateTime.parse(banlist.getString(event.name+".period"), DateTimeFormatter.ofPattern(options.getString("period-format"))).isAfter(LocalDateTime.now())) {
            event.disallow(
                AsyncPlayerPreLoginEvent.Result.KICK_BANNED,
                FormatManager.getBanFormat(
                    event.name,
                    banlist.getString("${event.name}.period")!!,
                    banlist.getString("${event.name}.reason")!!,
                    banlist.getString("${event.name}.time")!!,
                    banlist.getString("${event.name}.admin")!!
                )
            )
        } else {
